...
router.use(middlewares.isItHtmlGenerator);
	...
	([
		...
		function (callback)
		{
			DBMain.loc_byPath(path , encData.CollName , res.locals.lang, function(err, loc_path)
			{
				...
				{
					loc_path.splice(0,1);
					let location = [encData.title[res.locals.lang]].concat(loc_path);
		...
		function (callback)
		{
			DBArticles.article_resources_WUsersAResInfo(target_node_id, encData.CollName, res.locals.lang, function(err , res)
			{
				if(err || res == null)
				{
