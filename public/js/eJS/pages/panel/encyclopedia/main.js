function CreateArt(ev, cbf)
{
	var form = ev.target;
	CreateArt.artCretedMessage = CreateArt.artCretedMessage  || "مقاله ثبت شد";
	ev.preventDefault();
	$('#MModal').modal('show');
	tinyMCE.triggerSave();
	$.ajax
	({
		...
		success: function (res)
		{
			...
			{
				successMmodal(CreateArt.artCretedMessage);
				$('#CreEditArtSubmit').text('ویرایش');
				CreateArt.artCretedMessage = "ویرایش انجام شد";
				cbf(res);
			}
			else
			{
...
function DeleteArt(nodeId)
{
	...
	$.ajax
	({
		url: '/' + lang + '<%=consV.pages.panel.encyclopedia.deleteArt%>',
		type: 'POST',
		data:
		{
			nodeId: nodeId
	...
	function resursion(curNodeId, depth)
	{
		...
		else if(depth == max_depth - 1)
		{
			...
			onclick="maxDepthNodeClicked(event,'${nodes[curNodeId].data._id}')">\
				...
				<span id="chevron" style="margin-top: 7px; font-size: 12px;" class="fa fa-chevron-${direct}"></span>\
		...
		{
			ul_opened = true;
			html+= '<li class="cursorPointer" onmouseover="hover_on_item($(this));" onmouseleave="leave_item($(this))" onclick="collapse(event);">' + nodes[curNodeId].data.treeTitle[artLang] + `<span id="chevron" style="margin-top: 7px; font-size: 12px;" class="fa fa-chevron-${direct}"></span></li>`;
...
function CheckURLName(item)
{
	$.ajax
	({
		url: '/' + lang + '<%=consV.pages.panel.encyclopedia.URLNameValidation%>',
		type: 'POST',
		data:
		{
			nodeId: item._id,
