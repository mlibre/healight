...
function ArtResWithAddInfo(nodeId, nodeEnc, cbf)
{
	$.ajax
	({
		url: '/' + lang + '<%=consV.pages.panel.encyclopedia.ArtResources%>',
		type: 'POST',
		data:
		{
			nodeId: nodeId,
...
function deleteResource(ev)
{
	...
	$.ajax
	({
		url: '/' + lang + '<%=consV.pages.panel.encyclopedia.delResource%>',
		type: 'POST',
		data:
		{
			resId: resId,
...
function post_art_resources(ev)
{
	...
	$.ajax
	({
		url: '/' + lang + '<%=consV.pages.panel.encyclopedia.resources%>',
		type: 'POST',
		data: resData,
		success: function (res)
		{
			...
			else if( res == "<%=consV.codes.lackOfInformation%>" )
			{
