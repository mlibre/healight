...
function isSessCreated(req , res , next)
{
	if( typeof req.session == 'undefined')
	{
		...
		console.log('Probably you didnt run redis service. run: npm run srm'.green);
...
//**************** Permision & Access ****************\\
exports.isItHtmlGenerator = function (req , res , next)
{
	
	let url = req.originalUrl;
	if( (consV.project.html_gen == true) && (url.indexOf('?tree_nodeid') != -1) )
	{
		let noTreeNodIde = url.slice(0 , url.indexOf('?tree_nodeid'));
		console.log(url , noTreeNodIde);
		res.redirect(noTreeNodIde);
		next(new Error('redirected.'.red));
	}
	next();
}
